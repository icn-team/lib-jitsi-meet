...
export class MLSKeyHandler extends ExternallyManagedKeyHandler {
    constructor(conference) {
        ...
        this._maxClients = mls.maxClients;
        ...
        this.conference.on(
            ...
            this._onConferenceUniqueIdSet.bind(this));
        this.conference.on(
     ...
     * @private
     */
    async _onConferenceUniqueIdSet(meetingId) {
        logger.debug(`conference ID: ${meetingId}, user ID: ${this.conference.myUserId()}, max MLS clients: ${this._maxClients}`);
    }
    ...
    _isAuthorized() {
      ...
      for (const participant of this.conference.getParticipants()) {
          if (participant.getId() < this.conference.myUserId()) {
              smaller++;
          }
      }

      return smaller < this._maxClients;
    }
     ...
     */
    async _setKey() {
        let material = await importKey(new Uint8Array(KEY_SIZE));

        if (this._isAuthorized()) {
          material = await importKey(this._mlsAdapter.getKey());
        } else {
          logger.debug(`Client not authorized (max MLS clients: ${this._maxClients})`);
        }
